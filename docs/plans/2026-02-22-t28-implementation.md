# T28 Polish & Integration Testing — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development to implement this plan task-by-task.

**Goal:** Deliver Edit Visit (`app/visit/edit/[id].tsx`) plus three small polish fixes, completing the full CRUD flow for BrewLog.

**Architecture:** New standalone edit screen pre-filled from `getVisitWithDetails`, saving via `updateVisit` + drink/photo reconciliation. Polish fixes are isolated one-liner edits to existing screens.

**Tech Stack:** TypeScript, React Native / Expo SDK 54, Expo Router, expo-sqlite, expo-constants, react-native-safe-area-context

---

### Task 1: Polish fixes — SafeAreaView + error logging

**Files:**
- Modify: `app/visit/[id].tsx`
- Modify: `app/cafe/[id].tsx`
- Modify: `app/(tabs)/add.tsx`
- Modify: `src/components/CafeSearchBar.tsx`

**Step 1: Add SafeAreaView to `app/visit/[id].tsx`**

Add the import (it's not currently imported):

```ts
import { SafeAreaView } from 'react-native-safe-area-context';
```

Wrap the return JSX — replace:
...
Move `flex: 1, backgroundColor` from `container` to the `ScrollView` itself via `style={styles.scrollFill}`:
...
Apply: `<ScrollView style={styles.scrollFill} ...>`.

**Step 2: Add SafeAreaView to `app/cafe/[id].tsx`**

Same pattern. `cafe/[id].tsx` has `import { SafeAreaView } from 'react-native-safe-area-context';` — check first, add if missing.

Find the return's `<ScrollView` (around line 133) and wrap it:
...
**Step 3: Add error logging in `app/(tabs)/add.tsx`**

Find the two silent catches and add logging. There are exactly two:
...
With:
```ts
          handleCafeSelectFromGoogle(cafeData).catch((err) => console.error('Cafe select error:', err));
```
...
Wait — that's in `CafeSearchBar.tsx`. In `add.tsx`, the two silent catches are:

1. In the `useEffect` for cafe matching (search). Find `.catch(() => {` pattern in add.tsx and replace with `.catch((err) => console.error('[AddVisit] cafe search error:', err))`.

2. `handleCafeSelectFromGoogle(cafeData).catch(() => {})` → `.catch((err) => console.error('[AddVisit] cafe select error:', err))`.

**Step 4: Add error logging in `src/components/CafeSearchBar.tsx`**
...
Replace:
```ts
      handleSelectPrediction(prediction).catch(() => {});
```
With:
```ts
      handleSelectPrediction(prediction).catch((err) => console.error('[CafeSearchBar] select error:', err));
```

**Step 5: TypeScript check**

```bash
cd /Users/anthonymartino/Desktop/claude/brewlog && npx tsc --noEmit
```
...
```bash
git -C /Users/anthonymartino/Desktop/claude/brewlog add app/visit/[id].tsx app/cafe/[id].tsx 'app/(tabs)/add.tsx' src/components/CafeSearchBar.tsx
git -C /Users/anthonymartino/Desktop/claude/brewlog commit -m "fix: SafeAreaView on detail screens, add error logging to silent catches"
```

---

### Task 2: Wire edit button in Visit Detail

**Files:**
- Modify: `app/visit/[id].tsx`

**Step 1: Replace the console.log with router.push**
...
```bash
git -C /Users/anthonymartino/Desktop/claude/brewlog add 'app/visit/[id].tsx'
git -C /Users/anthonymartino/Desktop/claude/brewlog commit -m "fix: wire edit button to /visit/edit/:id route"
```

---

### Task 3: Create `app/visit/edit/[id].tsx`

**Files:**
- Create: `app/visit/edit/[id].tsx`

**Step 1: Create the directory and file**

Create `/Users/anthonymartino/Desktop/claude/brewlog/app/visit/edit/[id].tsx` with this exact content:
...
```bash
git -C /Users/anthonymartino/Desktop/claude/brewlog add 'app/visit/edit/[id].tsx'
git -C /Users/anthonymartino/Desktop/claude/brewlog commit -m "feat: add Edit Visit screen with full reconciliation (T28)"
```
...
**Step 1: Update the T26, T27, T28 rows**
...
```bash
git -C /Users/anthonymartino/Desktop/claude/brewlog add docs/STATUS.md
git -C /Users/anthonymartino/Desktop/claude/brewlog commit -m "docs: mark T26/T27/T28 done in STATUS.md"
```

---

### Task 5: Run Playwright tests and push
...
```bash
cd /Users/anthonymartino/Desktop/claude/brewlog && npx playwright test --config playwright.config.ts 2>&1
```
...
```bash
git -C /Users/anthonymartino/Desktop/claude/brewlog status
```
...
```bash
git -C /Users/anthonymartino/Desktop/claude/brewlog push -u origin feat/t28-polish
```
